...
class BootstrapWriter(BasicWriter):
    ...
    def rank_and_titles(self, rank_table, ranks_of_teams, teams=None, rank=None):
        ...
        html = "<ul class='nav nav-tabs'><li class='nav-item'><a class='nav-link active' data-toggle='tab' href='#seasons_" + str(random_id) + "'>Separate Seasons</a></li><li class='nav-item'><a class='nav-link' data-toggle='tab' href='#combinedtable_" + str(random_id) + "'>Combined Table</a></li></ul>"
    ...
    def multi_table(self, tables, teams=None, rank=None):
        ...
        for idx, (key, data) in enumerate(tables.items()):
            ...
            if idx == 0:
                html = html + f"<li class='nav-item'><a class='nav-link active' data-toggle='tab' href='#{data['id']}'>{data['name']}</a></li>"
            else:
                html = html + f"<li class='nav-item'><a class='nav-link' data-toggle='tab' href='#{data['id']}'>{data['name']}</a></li>"
