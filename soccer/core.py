...
class Soccer(object):
    ...
    def get_league_table(self, competition, season=None, matchday=None, sortBy=None, ascending=None, home=True, away=True, teams=[], head2headOnly=False):
        # sanity checks
        if sortBy is None:
        ...
        if not teams:
            # standard case: just load the table from fdo
            standings = self.fdo.get_league_table_by_league_code(competition, season, matchday)

        else:
            # load fixtures and compute table
            fixtures = self.fdo.get_fixtures_by_league_code(competition, season)
           
            standings = {
                "standing": self.compute_team_standings(fixtures, teams=teams, home=home, away=away, head2headOnly=head2headOnly)
            }
        
        if home and not away:
            standings = self.convert_league_table(standings)
        elif not home and away:
            standings = self.convert_league_table(standings, home=False)
        standings["standing"] = self.sort_league_table(standings["standing"], sortBy, ascending)
        return standings
    ...
    def compute_team_standings(self, fixtures, teams, home=True, away=True, head2headOnly=False):
        teamStandings = {}
        for team in teams:
            teamData = self.fdo.get_team(team)
